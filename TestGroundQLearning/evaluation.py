...
def evaluate_agent(Q_table_filename, human_jump, episode_num):
    ...
    for eval_ep in range(0,eval_eps):
        ...
        while not done: 
            human_robot_dist = np.linalg.norm(eval_env._agent_last_location - eval_env._human_location, ord=1)
            One_Action = [Q_table[0,current_human_state,current_agent_state],Q_table[1,current_human_state,current_agent_state]]

            if (human_robot_dist>2):
                if (np.all(One_Action)==0):
                    action = random.randint(0,1)
                else: 
                    action = np.argmax(One_Action)

            else: 
                #print("states: ", (current_human_state,current_agent_state))
                if (np.all(Q_table[:,current_human_state,current_agent_state])==0):
                    ...
                    #print("Q_table: ", Q_table[:,current_human_state,current_agent_state])
